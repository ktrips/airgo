...
**iPhone / iOS Safari の場合**: 本アプリはモバイルではリダイレクト方式でログインします。上記の設定（特に「承認済みのリダイレクト URI」）が正しく登録されていれば動作します。authDomain はアクセス中のドメインに自動設定されます。
...
**承認済みのリダイレクト URI** に以下を追加（iPhone 等モバイルでの Google ログインに必要）:

- `https://air.ktrips.net/__/auth/handler`
- `https://airgo.ktrips.net/__/auth/handler`
