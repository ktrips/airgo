...
function perfEnd(label) {
  if (_perfTimers[label]) {
    const duration = (performance.now() - _perfTimers[label]).toFixed(2);
...
function addPhotoMarkers() {
  perfStart('addPhotoMarkers');
  // 既存マーカーを一括削除（パフォーマンス最適化）
  markers.forEach(m => map.removeLayer(m));
  markers = [];
  _publicTripRouteLayers.forEach(l => { if (map && map.hasLayer(l)) map.removeLayer(l); });
  _publicTripRouteLayers = [];
...
function renderAllPhotosStrip() {
  perfStart('renderAllPhotosStrip');
  const strip = document.getElementById('allPhotosStrip');
  if (!strip) return;
  strip.innerHTML = '';
  ...
  photos.forEach((photo, i) => {
    ...
    if (hasPhotoData(photo)) {
      ...
      // 最初の20枚以外は遅延読み込み（Intersection Observer使用）
      if (i < 20) {
        img.src = photo.url || '';
      } else {
        img.dataset.lazySrc = photo.url || '';
        img.src = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"%3E%3C/svg%3E'; // 透明placeholder
...
/* === パフォーマンス最適化用の定数・キャッシュ === */
const THUMB_MAX_DIMENSION = 400; // サムネイル用の小さいサイズ
const THUMB_JPEG_QUALITY = 0.75; // サムネイル圧縮品質
const _blobUrlCache = new Map(); // base64→Blob URLのキャッシュ（重複変換を防止）
const _blobUrlCacheMaxSize = 150; // キャッシュの最大サイズ

/** デバウンス: 最後の呼び出しのみ実行（連続イベントを制御） */
...
/** 画像の遅延読み込み用 Intersection Observer */
...
async function loadTripAndShowPhoto(tripId, photoIndex) {
  perfStart('loadTripAndShowPhoto');
  cancelAddPointMode();
  if (tripId === '_other') {
    const allTrips = await getMergedTrips();
    const myTrips = allTrips.filter(t => !t.id?.startsWith('public_') && !t._isPublic);
...
async function renderPublicTripsPanel() {
  perfStart('renderPublicTripsPanel');
  ...
  perfEnd('renderPublicTripsPanel');
}
